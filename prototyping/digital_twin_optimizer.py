...
class DigitalTwinOptimizer:
    ...
    def optimize_recipe(self, target_mass_g: float = 4.0) -> Dict:
        ...
        energy_range = np.linspace(10.0, 20.0, 21)  # 0.5 MeV steps
        dose_range = np.logspace(np.log10(1.0), np.log10(100.0), 20)  # 1-100 kGy
        ...
        for energy in energy_range:
            for dose in dose_range:
                for composition in compositions:
                    ...
                    if yield_per_cad > best_yield:
                        best_yield = yield_per_cad
                        best_recipe = config.copy()
                    
                    if count % 100 == 0:
                        self.logger.info(f"Optimization progress: {count}/{total_combinations}")
        
        self.logger.info(f"Optimization complete. Best yield: {best_yield:.6f} mg Au/CAD")
